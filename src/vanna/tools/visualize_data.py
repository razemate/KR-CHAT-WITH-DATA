...
class VisualizeDataTool(Tool[VisualizeDataArgs]):
    ...
    async def execute(
        ...
        try:
            ...
            # Help the AI by listing available files
            try:
                available_files = await self.file_system.list_files(".", context)
                files_str = ", ".join(available_files) if available_files else "None"
            except:
                files_str = "Unknown (Error listing files)"

            error_message = f"File not found: {args.filename}. Available files in this session: {files_str}. Please use one of the available filenames."
            
            return ToolResult(
                success=False,
                result_for_llm=error_message,
                ui_component=None, # HIDE FROM USER: Keep UI clean while letting AI self-correct
                error=error_message,
                metadata={"error_type": "file_not_found", "available_files": available_files if 'available_files' in locals() else []},
            )
